    ...
    <div className={`navbar shadow-lg sticky top-0 z-50 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-base-100 text-black'}`}>
      <div className="navbar-start">
        <div className="dropdown">
          ...
          >
            {['Home', 'About', 'Tools & Technologies', 'Projects', 'Education', 'Contact'].map((item) => (
              <li key={item} className={`text-sm font-semibold ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-300'}`}>
                <a href={`/${item.toLowerCase().replace(/ & /g, '-').replace(/\s+/g, '-')}`}>{item}</a>
              </li>
            ))}
          </ul>
        </div>

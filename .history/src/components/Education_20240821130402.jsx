...
function Education({ isDarkMode }) {
  return (
    ...
    <div className={`flex  border border-slate-200 items-center  shadow-lg p-4 rounded-lg min-w-[70px] ${isDarkMode ? 'bg-transparent border-none' : 'bg-white'}`}>
    <img className={`rounded-lg w-16 h-16 mr-4  ${isDarkMode ? 'bg-slate-800' : 'bg-white'} `} src={Educ1} alt="React Js"/>
    <div className='flex-grow'> 
    
    <p className={`text-sm font-semibold ' ${isDarkMode ? 'text-white' : 'text-slate-400'}`}>Degree</p>
   
    <p className={`text-sm font-bold' ${isDarkMode ? 'text-white' : 'text-black'}`}>Bachelor of Science in Computer Science (BSCS)</p>

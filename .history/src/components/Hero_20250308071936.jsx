...
function Hero({ isDarkMode }) {
  return (
    <div name="Home" className={`hero min-h-screen relative ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
      {/* SVG Background Curved Lines */}es */}
      <svg className="absolute top-0 left-0 w-full h-full opacity-20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">">
        <path d="M0,50 C25,75 75,25 100,50" stroke={isDarkMode ? '#ffffff' : '#000000'} strokeWidth="2" fill="none" />00'} strokeWidth="2" fill="none" />
      </svg>
      <svg className="absolute bottom-0 right-0 w-full h-full opacity-20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">">
        <path d="M0,50 C25,25 75,75 100,50" stroke={isDarkMode ? '#ffffff' : '#000000'} strokeWidth="2" fill="none" />idth="2" fill="none" />
      </svg>

      ...
      <div className="max-w-screen-xl mx-auto py-8 px-8 lg:py-16 lg:px-6">
        ...
        <motion.div
          ...
          {tools.map((tool, index) => (
            <div key={index} className={`flex flex-col items-center ${isDarkMode ? 'bg-gray-700' : 'bg-slate-200'} p-2 rounded-lg mb-4 min-w-[70px]`}>
              <img className='mx-auto rounded-lg w-[48px]' src={tool.src} alt={tool.alt} />
              <p className={`text-sm text-center font-semibold ${isDarkMode ? 'text-white' : 'text-slate-600'}`}>{tool.label}</p>
            </div>
          ))}
        </motion.div>
      </div>

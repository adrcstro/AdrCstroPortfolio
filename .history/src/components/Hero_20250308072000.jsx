...
function Hero({ isDarkMode }) {
  return (
    <div name="Home" className={`hero min-h-screen relative ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
      ...
      <svg className="absolute top-0 left-0 w-1/2 h-1/2 opacity-20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r="40" fill={isDarkMode ? '#ffffff' : '#000000'} />
      </svg>
      <svg className="absolute bottom-0 right-0 w-1/3 h-1/3 opacity-20" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
        <rect x="10" y="10" width="80" height="80" fill={isDarkMode ? '#ffffff' : '#000000'} />
      </svg>
